...
// -l
void print_long_format(struct dirent *entry, const char *path, int show_size, int show_inode) {
    struct stat file_stat;
    char full_path[512];
    snprintf(full_path, sizeof(full_path), "%s/%s", path, entry->d_name);

    if (stat(full_path, &file_stat) == 0) {    // 打印文件信息
...
// -R
void list_directory(const char *path, int show_long, int show_hidden, int show_recursive, int show_inode, int show_size, int sort_by_time, int reverse, int show_time) {
    ...
    for (int i = 0; i < count; ++i) {
        if (show_long) {
            print_long_format(entries[i], path, show_size, show_inode);
        } else {
    ...
    if (show_recursive) {
        for (int i = 0; i < count; ++i) {
            ...
            if (stat(full_path, &file_stat) == 0 && S_ISDIR(file_stat.st_mode)) {
                printf("\n%s:\n", entries[i]->d_name);
                list_directory(full_path, show_long, show_hidden, show_recursive, show_inode, show_size, show_time, reverse, show_time);
            }
        }
    }
...
void sort_and_list(const char *path, int show_long, int show_hidden, int show_recursive, int reverse, int show_inode, int show_size, int show_time) {
    list_directory(path, show_long, show_hidden, show_recursive, show_inode, show_size, show_time, reverse, show_time);
}
